...
class EnhancedRAGSystem:
    ...
    def _initialize_settings(self):
        """Initialize LlamaIndex settings with Groq only"""
    ...
    def extract_text_from_txt(self, path: str) -> str:
        ...
        for encoding in encodings:
            try:
                ...
                continue
        logging.error("TXT extraction failed for all encodings")
        return ""
        ...
        try:
            ...
            if top_score < SIMILARITY_THRESHOLD and len(retrieved_nodes) > K_FINAL:
                try:
                    ...
                    scored_nodes = sorted(zip(retrieved_nodes[:K_RERANK], scores), key=lambda x: x[1], reverse=True)
            ...
            response_obj = Settings.llm.complete(prompt_text)
...
def search():
    """Search endpoint — all languages handled by Groq"""
    try:
        start_time = time.time()
        ...
        # If RAG returned a good answer, enrich it with conversation history via Groq
        if rag_context and "not available" not in rag_context.lower():
            if conversation_history and groq_client:
                try:
        ...
        # Fallback: direct Groq chat (all languages)
        if not groq_client:
            return jsonify({
                "success": False,
                "error": "Groq client not available. Please set a valid GROQ_API_KEY.",
                "response": "Service unavailable. Please check your API configuration."
            }), 500

        system_prompt = prompt.prompts
        ...
        try:
            logging.info(f"Using Groq for {detected_lang} with conversation history")
...
def health():
    return jsonify({
        ...
        "all_languages_support": groq_client is not None
    })
