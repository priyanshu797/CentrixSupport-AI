...
class MultiLevelCache:
    ...
    def __init__(self, cache_dir=CACHE_DIR, max_size=CACHE_SIZE):
...
def query_index_optimized(index, query: str) -> Tuple[str, List[str], Dict[str, float]]:
    timings = {}
    ...
    if top_score < SIMILARITY_THRESHOLD and len(retrieved_nodes) > K_FINAL:
        try:
            ...
            logging.warning(f"Reranking failed: {e}. Using top-k nodes.")
    ...
    if any(phrase in response.lower() for phrase in generic_phrases) and len(response.split()) < 30:
...
def main():
    ...
    while True:
        ...
        response, sources, timings = query_index_optimized(index, question)
