...
def search():
    ...
    try:
        ...
        # RAG processing with enhanced file handling
        rag_context = ""
        rag_sources = []
        used_files = []
        ...
        # If we got a RAG response and it's not just "no info", return it
        if rag_context and not "not available" in rag_context.lower():
            # Build context-aware response with history
        ...
        # If no RAG context or RAG said "no info", fall back to regular chat
        system_prompts = {
